...
class _MapScreenState extends State<MapScreen> {
  ...
  void _onSearchChanged() {
    setState(() { _searchQuery = _searchController.text; _applyFilters(); });
  }
  ...
  void _applyFilters() {
    ...
    if (_sortBy == 'rating') result.sort((a, b) => b.rating.compareTo(a.rating));
    else result.sort((a, b) => a.distance.compareTo(b.distance));
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _FilterBottomSheet(
        ...
        onApply: (available, sort) {
          setState(() { _showAvailableOnly = available; _sortBy = sort; _applyFilters(); });
        },
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          _isLoading ? const Center(child: CircularProgressIndicator(color: _teal)) : _buildMap(),
          _MapTopOverlay(
  ...
  List<Marker> _buildMarkers() {
    ...
    for (final provider in _filteredProviders) {
      markers.add(Marker(
        ...
        child: GestureDetector(onTap: () => _showProviderBottomSheet(provider), child: _ProviderMapPin(provider: provider)),
      ));
    }
    if (_currentPosition != null) {
      markers.add(Marker(width: 28, height: 28, point: _currentPosition!, child: _UserLocationPin()));
    }
    return markers;
  }
    ...
    showModalBottomSheet(
      ...
      builder: (context) => _ProviderBottomSheet(
        ...
        onViewDetails: () {
          ...
          Navigator.push(context, MaterialPageRoute(builder: (_) => const ProviderDetailsScreen(), settings: RouteSettings(arguments: provider)));
        },
      ),
    );
  }
...
// ── Map pin & provider sheet (unchanged logic) ────────────────────────────────
    ...
    return Stack(
      ...
      children: [
        Container(
          ...
          child: Center(child: provider.isAvailable ? const Icon(Icons.local_hospital_rounded, color: Colors.white, size: 20) : const Icon(Icons.add_box_rounded, color: Colors.white, size: 20)),
        ),
