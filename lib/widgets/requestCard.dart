...
//     final createdAt = request.timestamp;
//     Duration? remaining;
//
//     if (createdAt != null && request.status == 'pending') {
//       remaining = ExpirationLogicService.getRemainingTime(createdAt as Timestamp);
//     }
//     return Card(
...
class RequestCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final myOffer = offers
        .where((o) => o.providerId == currentUserId)
        .cast<RequestOffer?>()
        .firstWhere(
          (o) => o != null,
      orElse: () => null,
    ...
    final bool hasOffered = myOffer != null;

    final bool isWinner =
        request.status == 'confirmed' &&
            request.confirmedProviderId == currentUserId;

    final bool someoneElseWon =
        request.status == 'confirmed' &&
            request.confirmedProviderId != currentUserId;

    final bool isExpired = request.status == 'expired';
                ...
                if (isCompleted)
                  ...
                  _buildExpiredState()
                    ...
                    _buildWinnerState()
                      ...
                      _buildOtherProviderSelectedState()
                    else if (isRejected)
                        _buildRejectedState()
                      else if (isWaiting)
                          _buildWaitingState()
  ...
  Widget _buildStatusHeader(
    ...
    if (isCompleted) {
      ...
      label = "ANOTHER PROVIDER SELECTED";
      color = Colors.grey.shade600;
    } else if (isRejected) {
  ...
  Widget _buildOtherProviderSelectedState() {
    return const Center(
      child: Text(
        "Another provider has been selected.",
        style: TextStyle(
            color: Colors.grey),
      ),
    );
  ...
  Widget _buildExpiredState() {
    return const Center(
      child: Text(
        "This request expired due to no confirmation.",
        style: TextStyle(
            color: Colors.grey),
      ),
    );
